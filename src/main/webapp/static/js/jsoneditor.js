...
function jsonEditorInit(table_container_id, json_input_container_id, json_output_container_id, json_to_table_btn_id, table_to_json_btn_id) {
    ...
    $('#' + json_to_table_btn_id).on('click', function(){
        ...
        try {
            ...
            var groupName = document.querySelector(".nameGroup").textContent;
            var scheduleType = document.querySelector(".typeSchedule").textContent;

            var urlGroup = "../resources/json/" + scheduleType + "/" + groupName + ".json";
            var urlDefault = "../resources/json/" + scheduleType + "/" + "schedule" +".json";

            if (!UrlExists(urlGroup)) {
                var request = new XMLHttpRequest();
                request.open("GET", urlDefault, false);
                request.send(null);
            }
            else {
                var request = new XMLHttpRequest();
                request.open("GET", urlGroup, false);
                request.send(null);
            }
